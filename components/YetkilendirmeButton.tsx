...
const MESSAGE_TEMPLATE = "VIP Katalog için yetkilendirme talep ediyorum. Telefon numaram: ";
...
export function YetkilendirmeButton({ phoneNumber = "", compact }: YetkilendirmeButtonProps) {
  ...
  const handleClick = () => {
    if (!canOpen) return;
    const message = `${MESSAGE_TEMPLATE}${formattedPhone}`;
    window.open(
    ...
    return (
      <div className="flex flex-col gap-2">
        ...
        <button
          ...
          onClick={handleClick}
          disabled={!canOpen}
          className="inline-flex items-center justify-center gap-2 rounded-lg border border-green-600/50 bg-green-600/10 py-2 px-4 text-sm font-medium text-green-400 transition hover:bg-green-600/20 disabled:cursor-not-allowed disabled:opacity-50"
        >
  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-zinc-700/50 bg-zinc-800/30 p-4">
      ...
      <button
        ...
        onClick={handleClick}
        disabled={!canOpen}
        className="inline-flex items-center justify-center gap-2 rounded-lg border border-green-600/50 bg-green-600/10 py-3 px-4 font-medium text-green-400 transition hover:bg-green-600/20 disabled:cursor-not-allowed disabled:opacity-50"
      >
